...
def process(carPaths, roads):
    ...
    out = []
    for junctionid, incomingRoads in intersectionIncomingMap.items():
        temp = []
        for road in incomingRoads:
            if road in criticalPath:
                temp.append((road, criticalPath[road]))
        out.append((junctionid, temp))

    return out

def readInput():
    inputFile = open("C:\\Users\\deblghosh\\OneDrive - Lexmark\\Chrome\\d.txt", "r")
...
def processOutput(output):
    out = open(f'C:\\Users\\deblghosh\\OneDrive - Lexmark\\Chrome\\dOut.txt', 'w')
